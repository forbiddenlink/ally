...
<html>
  ...
  describe('Confidence Thresholds', () => {
    it('should apply fixes with ≥90% confidence by default', () => {
      const fixes = [
        ...
        { pattern: 'image-alt', confidence: 0.9, shouldApply: true },
        { pattern: 'button-name', confidence: 0.85, shouldApply: false },
        { pattern: 'complex-focus', confidence: 0.6, shouldApply: false },
      ]

      const applied = fixes.filter((f) => f.confidence >= 0.9)
  ...
  describe('Watch Mode Output', () => {
    it('should show real-time fix status', () => {
      ...
      assert.strictEqual(output.threshold, 0.9, 'Should use 90% threshold')
    })
  ...
  describe('File Pattern Matching', () => {
    it('should watch HTML files by default', () => {
      ...
      const testFiles = ['index.html', 'about.htm', 'style.css', 'script.js']

      const matches = testFiles.filter((f) => patterns.some((p) => f.match(/\.(html|htm)$/)))
    ...
    it('should support custom file patterns', () => {
      ...
      const testFiles = ['Button.tsx', 'Modal.jsx', 'utils.ts', 'index.html']

      const matches = testFiles.filter((f) => customPatterns.some((p) => f.match(/\.(tsx|jsx)$/)))
...
`
      ...
      const allFiles = ['src/index.html', 'dist/index.html', 'build/app.html', 'test.test.html']

      // Simulate filtering
      const ignored = ['dist/', 'build/', '.test.html']
      const watched = allFiles.filter(
        (f) =>
  ...
  describe('Performance', () => {
    ...
    it('should limit concurrent scans', () => {
      ...
      const queue = ['file1.html', 'file2.html', 'file3.html', 'file4.html', 'file5.html']
