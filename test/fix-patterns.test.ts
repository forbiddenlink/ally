...
describe('Helper Functions', () => {
  ...
  describe('inferDescription', () => {
    ...
    it('should use text content', () => {
      assert.strictEqual(
        inferDescription('<button>Submit Form</button>', 'fallback'),
        'Submit Form'
      )
    })
...
describe('Fix Patterns', () => {
  describe('image-alt', () => {
    it('should add alt from filename', () => {
      const result = FIX_PATTERNS['image-alt'](
        '<img src="/images/hero-banner.jpg">',
        mockViolation('image-alt')
      )
  ...
  describe('image-redundant-alt', () => {
    it('should remove "image of" prefix', () => {
      const result = FIX_PATTERNS['image-redundant-alt'](
        '<img alt="image of a cat">',
        mockViolation('image-redundant-alt')
      )
  ...
  describe('button-name', () => {
    it('should add aria-label from content', () => {
      const result = FIX_PATTERNS['button-name'](
        '<button>Submit</button>',
        mockViolation('button-name')
      )
  ...
  describe('link-name', () => {
    it('should add aria-label from href', () => {
      const result = FIX_PATTERNS['link-name'](
        '<a href="/about-us"></a>',
        mockViolation('link-name')
      )
  ...
  describe('html-has-lang', () => {
    ...
    it('should return null if lang exists', () => {
      const result = FIX_PATTERNS['html-has-lang'](
        '<html lang="fr">',
        mockViolation('html-has-lang')
      )
      assert.strictEqual(result, null)
    })
  })
  ...
  describe('document-title', () => {
    it('should add title element', () => {
      const result = FIX_PATTERNS['document-title'](
        '<head></head>',
        mockViolation('document-title')
      )
  ...
  describe('meta-viewport', () => {
    it('should remove user-scalable=no', () => {
      const result = FIX_PATTERNS['meta-viewport'](
        '<meta name="viewport" content="width=device-width, user-scalable=no">',
        mockViolation('meta-viewport')
      )
  ...
  describe('label', () => {
    it('should add aria-label from name', () => {
      const result = FIX_PATTERNS['label'](
  ...
  describe('autocomplete-valid', () => {
    it('should add autocomplete="email" for email input', () => {
      const result = FIX_PATTERNS['autocomplete-valid'](
        '<input type="email">',
        mockViolation('autocomplete-valid')
      )
  ...
  describe('frame-title', () => {
    it('should add title from src', () => {
      const result = FIX_PATTERNS['frame-title'](
        '<iframe src="/embed/video-player.html">',
        mockViolation('frame-title')
      )
  ...
  describe('aria-hidden-focus', () => {
    it('should add tabindex="-1" to aria-hidden element', () => {
      const result = FIX_PATTERNS['aria-hidden-focus'](
        '<button aria-hidden="true">',
        mockViolation('aria-hidden-focus')
      )
  ...
  describe('svg-img-alt', () => {
    it('should add role and aria-label', () => {
      const result = FIX_PATTERNS['svg-img-alt'](
        '<svg viewBox="0 0 100 100">',
        mockViolation('svg-img-alt')
      )
  ...
  describe('video-caption', () => {
    it('should add captions track', () => {
      const result = FIX_PATTERNS['video-caption'](
        '<video src="movie.mp4"></video>',
        mockViolation('video-caption')
      )
  ...
  describe('link-in-text-block', () => {
    it('should add underline style', () => {
      const result = FIX_PATTERNS['link-in-text-block'](
        '<a href="#">Link</a>',
        mockViolation('link-in-text-block')
      )
