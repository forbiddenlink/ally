...
import { calculateSummary, generateVpatHtml } from '../dist/utils/vpat-template.js'
...
describe('VPAT mappings', () => {
  describe('WCAG_CRITERIA', () => {
    ...
    it('should have axe rules for testable criteria', () => {
      ...
      for (const criterion of testable) {
        assert.ok(criterion.axeRules.length > 0, `Criterion ${criterion.id} should have axe rules`)
      }
    })
    ...
    it('should have manual notes for non-automatable criteria', () => {
      ...
      for (const criterion of manual) {
        assert.ok(criterion.manualNote, `Criterion ${criterion.id} should have manual note`)
      }
    })
  })
  ...
  describe('getCriteriaByLevel', () => {
    it('should return only Level A for A', () => {
      ...
      assert.ok(
        criteria.every((c) => c.level === 'A'),
        'All should be Level A'
      )
    })
...
describe('VPAT template', () => {
  describe('calculateSummary', () => {
    it('should count statuses correctly', () => {
      ...
      const total =
        summary.supports +
        summary.partiallySupports +
        summary.doesNotSupport +
