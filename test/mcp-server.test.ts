...
`
  ...
  describe('get_scan_summary', () => {
    it('should return summary of accessibility issues', async () => {
      ...
      await writeFile(join(testDir, '.ally', 'scan.json'), JSON.stringify(scanData, null, 2))
  ...
  describe('Integration: Full Workflow', () => {
    it('should support scan -> analyze -> fix workflow', async () => {
      ...
      const patterns = [{ component: 'Button', patterns: { hasAriaLabel: true } }]
      ...
      // 4. Update history
      await writeFile(join(testDir, '.ally', 'fix-history.json'), JSON.stringify(fixes))
