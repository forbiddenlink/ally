...
async function benchmarkPa11y(): Promise<BenchmarkResult> {
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    const result = await runCommand('pa11y', [DEMO_FILE, '--reporter', 'json'], (output) => {
      try {
        const data = JSON.parse(output)
...
async function benchmarkAxe(): Promise<BenchmarkResult> {
  ...
  for (let i = 0; i < ITERATIONS; i++) {
    const result = await runCommand('axe', [DEMO_FILE, '--reporter', 'json'], (output) => {
      try {
        const data = JSON.parse(output)
...
async function main(): Promise<void> {
  ...
  // Run benchmarks
  const results = await Promise.all([benchmarkAlly(), benchmarkPa11y(), benchmarkAxe()])
  ...
  if (allyResult?.available && baseline) {
    regressionPercent = ((allyResult.avgTime - baseline.allyAvgTime) / baseline.allyAvgTime) * 100
  }
  ...
  if (jsonMode) {
    ...
    if (regressionPercent !== null) {
      ...
      if (regressionPercent > 0) {
        console.log(
          chalk.yellow(`  Performance degraded by ${regressionPercent.toFixed(1)}% vs baseline`)
        )
      } else {
        console.log(
          chalk.green(
