...
const telemetry = {
  ...
  log(toolName: string) {
    ...
    console.error(
      `[MCP Telemetry] ${new Date().toISOString()} | ${toolName} called (${count + 1}x total)`
    )
  },
}
...
server.tool(
  ...
  'Analyze existing ARIA patterns in React/Vue components to ensure consistent accessibility fixes',
  {
    directory: z
      .string()
      .optional()
      .describe('Directory to analyze (defaults to current directory)'),
    ...
    try {
      ...
      if (wcagCompliantColors.length > 0) {
        ...
        response += wcagCompliantColors
          .map(
            (c) => `- ${c.name}: ${c.value} (contrast: ${c.contrastRatio?.toFixed(1) || 'unknown'})`
          )
          .join('\n')
        response += '\n\n'
      }
...
server.tool(
  ...
  'Retrieve previously applied accessibility fixes to maintain consistency',
  {
    issueType: z
      .string()
      .optional()
      .describe("Filter by issue type (e.g., 'image-alt', 'button-name')"),
  },
  async ({ issueType }) => {
...
const wcagCriteria: Record<string, WcagCriterion> = {
  '1.1.1': {
    ...
    level: 'A',
    description:
      'All non-text content that is presented to the user has a text alternative that serves the equivalent purpose, except for controls, input, time-based media, tests, sensory experiences, CAPTCHA, and decoration.',
    techniques: [
  ...
  '1.3.5': {
    ...
    failures: ['Failure to use autocomplete attribute when collecting user information'],
  ...
  '1.4.11': {
    ...
    failures: ['F78: Failure due to using purely decorative elements with insufficient contrast'],
  ...
  '2.4.1': {
    ...
    failures: ['No specific failures defined'],
  ...
  '2.4.2': {
    ...
    failures: ['F25: Failure due to the title of a Web page not identifying the contents'],
  ...
  '2.4.6': {
    ...
    techniques: ['G130: Providing descriptive headings', 'G131: Providing descriptive labels'],
    failures: ['No specific failures defined'],
  ...
  '3.1.1': {
    ...
    techniques: ['H57: Using the language attribute on the HTML element'],
    failures: ['No specific failures defined'],
  ...
  '3.1.2': {
    ...
    techniques: ['H58: Using language attributes to identify changes in the human language'],
    failures: ['No specific failures defined'],
...
const ariaPatterns: Record<string, AriaPattern> = {
  modal: {
    name: 'Modal Dialog',
    description:
      'A dialog is a window overlaid on the primary window. Focus is trapped within the dialog until it is dismissed.',
    ...
    keyboardInteractions: [
      {
        key: 'Tab',
        action: 'Move focus to next focusable element inside dialog (wraps to first when at end)',
      },
      {
        key: 'Shift+Tab',
        action:
          'Move focus to previous focusable element inside dialog (wraps to last when at start)',
      },
...
tablist.addEventListener('keydown', (e) => {
  ...
  menu: {
    ...
    states: [
      ...
      "aria-checked='true|false' (for menuitemcheckbox/radio)",
    ],
...
input.addEventListener('keydown', (e) => {
  ...
  tooltip: {
    name: 'Tooltip',
    description:
      'A popup displaying a description for an element when the element receives keyboard focus or hover.',
    roles: ['tooltip'],
    states: [],
...
[role="tooltip"] {
  ...
  alert: {
    name: 'Alert',
    description:
      'A live region containing important, time-sensitive information. Alerts are assertive and announced immediately.',
    ...
    keyboardInteractions: [
      {
        key: 'None required',
        action: 'Alerts do not require keyboard interaction - they are announced automatically',
      },
    ],
...
function showAlert(message) {
  ...
  tree: {
    name: 'Tree View',
    description:
      'A hierarchical list that allows users to expand and collapse nested groups of items.',
...
server.tool(
  ...
  'Get full WCAG success criterion details for a violation',
  {
    criterionId: z
      .string()
      .describe("WCAG criterion like '1.1.1' or violation id like 'image-alt'"),
  },
  async ({ criterionId }) => {
...
server.tool(
  ...
  'Calculate WCAG contrast ratio and APCA Lc value between two colors and check compliance',
  {
    ...
    fontSize: z
      .number()
      .optional()
      .describe('Font size in pixels (optional, for AA/AAA thresholds)'),
  ...
  async ({ foreground, background, fontSize, isBold }) => {
    ...
    try {
      ...
      if (passesAAA) {
        recommendation =
          'Excellent! This color combination meets WCAG AAA standards for all text sizes.'
      } else if (passesAA) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (
            componentFilter &&
            !entry.name.toLowerCase().includes(componentFilter.toLowerCase())
          ) {
            continue
          }
