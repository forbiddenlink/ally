...
import { animateScoreUp, printBanner, printError, printInfo, printSuccess } from '../utils/ui.js'
...
export async function statsCommand(): Promise<void> {
  ...
  if (history.length > 0) {
    ...
    for (const entry of recentHistory) {
      ...
      const scoreColor =
        entry.score >= 75 ? chalk.green : entry.score >= 50 ? chalk.yellow : chalk.red
      const bar = getSmallBar(entry.score)
      const issues = entry.errors + entry.warnings
      console.log(
        `  ${chalk.dim(date)}  ${bar}  ${scoreColor(entry.score.toString().padStart(3))}  (${issues} issues)`
      )
    }
    ...
    const currentColor =
      report.summary.score >= 75
        ...
        : report.summary.score >= 50
          ...
          : chalk.red
    console.log(
      `  ${chalk.bold('Today')}       ${currentBar}  ${currentColor.bold(report.summary.score.toString().padStart(3))}  (${report.summary.totalViolations} issues)`
    )
  ...
  if (report.summary.score === 100) {
    console.log(
      boxen(chalk.green.bold('🏆 Perfect Score! Your site is fully accessible.'), {
