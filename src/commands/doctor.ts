...
async function checkPuppeteerBrowser(): Promise<CheckResult> {
  try {
    ...
    if (
      ...
      return {
        status: 'fail',
        message: 'Puppeteer browser not installed. Run: npx puppeteer browsers install chrome',
      }
    }
  ...
  if (existsSync(mcpConfigPath)) {
    try {
      ...
      if (config.mcpServers && config.mcpServers['ally-patterns']) {
        return {
          status: 'warn',
          message: 'MCP config exists but server files not found. Run: npm run build:all',
        }
      }
    } catch {
