...
import type { AllyReport, ScanResult } from '../types/index.js'
import { isTransientError, withRetry } from '../utils/retry.js'
import { AccessibilityScanner, calculateScore, createReport } from '../utils/scanner.js'
import {
  createSpinner,
  printBanner,
  printError,
...
export async function crawlCommand(
  ...
  try {
          ...
          for (const link of links) {
            ...
            if (!visited.has(normalizedLink) && isValidPageUrl(normalizedLink)) {
              if (!sameOrigin || isSameOrigin(normalizedStartUrl, normalizedLink)) {
                ...
                const alreadyQueued = toVisit.some(
                  (item) => normalizeUrl(item.url) === normalizedLink
                )
                if (!alreadyQueued) {
    ...
    // Print crawl summary
    console.log(
      chalk.bold(`Crawl complete: ${results.length} page${results.length === 1 ? '' : 's'} scanned`)
    )

    // Calculate totals
    const totalViolations = results.reduce((sum, r) => sum + r.violations.length, 0)
    const avgScore = Math.round(
      results.reduce((sum, r) => sum + calculateScore([r]), 0) / results.length
    )
...
async function saveReport(report: AllyReport, outputDir: string, startUrl: string): Promise<void> {
  try {
