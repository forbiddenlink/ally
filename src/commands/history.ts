...
function printEntry(entry: HistoryEntry, index: number, total: number): void {
  ...
  const branchInfo = entry.branch
    ? chalk.dim(` [${entry.branch}`) +
      (entry.commit ? chalk.dim(`@${entry.commit}]`) : chalk.dim(']'))
    : ''
...
export async function historyCommand(options: HistoryCommandOptions = {}): Promise<void> {
  ...
  if (entries.length > 1) {
    ...
    console.log(
      chalk.dim(`${entries[0].score}${' '.repeat(42)}${entries[entries.length - 1].score}`)
    )
    console.log()
  }
