...
import type { AllyReport, ScanResult, Severity } from '../types/index.js'
...
async function fetchStoriesIndex(storybookUrl: string): Promise<StorybookEntry[]> {
  ...
  try {
    ...
    if (indexResponse.ok) {
      ...
      const entries = Object.values(index.entries).filter((entry) => entry.type === 'story') // Filter out docs
      return entries
    }
  } catch {
...
function filterStories(stories: StorybookEntry[], pattern: string): StorybookEntry[] {
  ...
  return stories.filter(
    (story) => regex.test(story.title) || regex.test(story.name) || regex.test(story.id)
  )
}
...
function printStorybookSummary(
  ...
  console.log(
    `  With issues: ${chalk[withIssues > 0 ? 'yellow' : 'green'](withIssues.toString())} (${issuesPercent}%)`
  )
  console.log()
  console.log(
    `Total: ${chalk.red(`${totalErrors} error${totalErrors !== 1 ? 's' : ''}`)}, ${chalk.yellow(`${totalWarnings} warning${totalWarnings !== 1 ? 's' : ''}`)}`
  )
}
    ...
    for (const [componentTitle, componentStories] of groupedStories) {
      ...
      for (const story of componentStories) {
        ...
        try {
          ...
          if (errors > 0) {
            scanSpinner.fail(
              `${componentTitle}/${story.name}: ${errors} errors, ${warnings} warnings`
            )
          } else if (warnings > 0) {
