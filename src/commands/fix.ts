...
async function applyFix(file: string, violation: Violation): Promise<FixResult> {
  ...
  if (suggestedFix) {
    ...
    return {
      ...
      line: 0,
      violation: violation.id,
      fixed: false, // Not actually fixed without Copilot
      skipped: false,
    };
  }
...
function generateSuggestedFix(violation: Violation, html: string): string | null {
  ...
  const fixes: Record<string, (html: string) => string | null> = {
    'image-alt': (h) => {
      if (h.includes('alt=')) return null;
      return h.replace(/<img/, '<img alt="[describe image]"');
    },
    'button-name': (h) => {
      if (h.includes('aria-label')) return null;
      return h.replace(/<button/, '<button aria-label="[action description]"');
    },
    'link-name': (h) => {
      if (h.includes('aria-label')) return null;
      return h.replace(/<a/, '<a aria-label="[link description]"');
    },
    'html-has-lang': (h) => {
      if (h.includes('lang=')) return null;
      return h.replace(/<html/, '<html lang="en"');
    },
  };

  const fixer = fixes[violation.id];
  return fixer ? fixer(html) : null;
}
