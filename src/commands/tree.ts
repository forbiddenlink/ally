...
function formatProperties(node: AXNode): string {
  ...
  if (node.pressed !== undefined) {
    props.push(
      node.pressed === 'mixed' ? 'pressed=mixed' : node.pressed ? 'pressed' : 'not pressed'
    )
  }
  if (node.selected) {
    props.push('selected')
...
function printNode(
  ...
  if (node.children && node.children.length > 0) {
    if (currentDepth >= maxDepth) {
      ...
      const childPrefix = isLast ? `${prefix}    ` : `${prefix}${TREE_CHARS.pipe}   `
...
function announceNode(node: AXNode): string {
  ...
  switch (role) {
    ...
    case 'list': {
      const itemCount = node.children?.filter((c) => (c as AXNode).role === 'listitem').length || 0
      if (itemCount > 0) {
...
export async function treeCommand(url: string, options: TreeCommandOptions = {}): Promise<void> {
  printBanner()
  ...
  try {
    ...
    if (actualDepth > depth) {
      printWarning(
        `Tree depth (${actualDepth}) exceeds display limit (${depth}). Use --depth to show more.`
      )
      console.log()
    }
    ...
    console.log(
      chalk.dim(`  ${chalk.cyan('role')} ${chalk.white('"name"')} ${chalk.gray('[properties]')}`)
    )
    console.log()
    ...
    if (speak) {
      ...
      const treeToAnnounce = role ? filterByRole(snapshot as AXNode, role) : [snapshot as AXNode]
      ...
      console.log(
        boxen(chalk.white(`"${announcementText}"`), {
          padding: { top: 0, bottom: 0, left: 1, right: 1 },
          borderStyle: 'round',
          borderColor: 'yellow',
        })
