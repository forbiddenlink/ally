...
import { createSpinner, printBanner, printError, printInfo } from '../utils/ui.js'
...
const CATEGORY_CONFIG: Record<Category, CategoryConfig> = {
  ...
  'color-contrast': {
    name: 'Color Contrast',
    icon: '2',
    patterns: [/^color-contrast/, /^link-in-text-block$/],
  },
  'keyboard-nav': {
    name: 'Keyboard Nav',
    icon: '3',
...
function generateRecommendations(
  ...
  const categoryViolations: Map<
    Category,
    Map<string, { count: number; description: string }>
  > = new Map()
...
function renderHealthBox(
  ...
  const scoreColor =
    overallScore >= 75 ? chalk.green : overallScore >= 50 ? chalk.yellow : chalk.red
