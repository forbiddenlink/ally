...
import type { AllyReport, ReportSummary, ScanResult, Severity, Violation } from '../types/index.js'
import {
  type BrowserAdapter,
  type BrowserType,
  createBrowser,
  ...
  const axeResults = await underlyingPage.evaluate((runTags) => {
    ...
    return (
      window as unknown as {
        axe: { run: (options: { runOnly: { type: string; values: string[] } }) => Promise<unknown> }
      }
    ).axe.run({
      runOnly: {
        type: 'tag',
...
export class AccessibilityScanner {
  ...
  async scanHtmlString(
    html: string,
    identifier: string = 'inline',
    standard: WcagStandard = DEFAULT_STANDARD
  ): Promise<ScanResult> {
    const tags = standardToTags[standard]
...
export function generateSummary(results: ScanResult[]): ReportSummary {
  ...
  const issueCount: Map<string, { count: number; description: string; severity: Severity }> =
    new Map()
 ...
 */
export async function runPa11yAudit(url: string, options: Pa11yOptions = {}): Promise<ScanResult> {
  const {
    standard = 'WCAG2AA',
