...
import type { ReportSummary, Severity, Violation } from '../types/index.js'
...
export function printViolation(
  ...
  if (impactScore) {
    const scoreColor =
      impactScore.score >= 95
        ? chalk.red.bold
        : impactScore.score >= 75
          ? chalk.red
  ...
  if (impactScore) {
    ...
    console.log(chalk.dim(`      👥 Affects: `) + chalk.cyan(impactScore.affectedUsers.join(', ')))
    console.log(chalk.dim(`      📊 Estimated: `) + chalk.cyan(impactScore.estimatedUsers))
  }
  ...
  violation.nodes.forEach((node, i) => {
    if (i < 3) {
      // Limit to 3 nodes for readability
...
export function printScoreChange(before: number, after: number): void {
  ...
  console.log(
    boxen(`Score: ${before} → ${chalk.bold(after.toString())} (${diffStr})`, {
...
export function printScoreBadge(score: number): void {
  ...
  console.log(
    boxen(`${emoji} Accessibility Score: ${color.bold(`${score}/100`)}`, {
      padding: { top: 0, bottom: 0, left: 1, right: 1 },
      borderStyle: 'round',
      borderColor: score >= 75 ? 'green' : score >= 50 ? 'yellow' : 'red',
    })
