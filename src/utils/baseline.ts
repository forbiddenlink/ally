...
export async function saveBaseline(
  report: AllyReport,
  baselineDir: string = BASELINE_DIR
): Promise<void> {
...
export function formatRegression(analysis: RegressionAnalysis): string {
  ...
  if (analysis.improved > 0) {
    lines.push(
      `✅ Fixed:      ${analysis.improved} file${analysis.improved === 1 ? '' : 's'} (${analysis.improvementPercentage.toFixed(1)}%)`
    )
  }
  if (analysis.regressed > 0) {
    lines.push(
      `⚠️  Regressed:  ${analysis.regressed} file${analysis.regressed === 1 ? '' : 's'} (${analysis.regressionPercentage.toFixed(1)}%)`
    )
  }
